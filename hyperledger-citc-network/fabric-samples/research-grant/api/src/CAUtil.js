...
exports.enrollAdmin = async (caClient, wallet, orgMspId) => {
	try {
		...
		console.log(`Getting user=`+adminUserId);
		...
		const x509Identity = {
			...
			mspId: orgMspId[0].toUpperCase() + orgMspId.slice(1)+'MSP',
			type: 'X.509',
		};
...
exports.registerAndEnrollUser = async (caClient, wallet, orgMspId, userId, affiliation) => {
	try {
		...
		return {"status": `Successfully registered and enrolled user ${userId} and imported it into the wallet`}
	} catch (error) {
...
exports.userExist=async(wallet,userId)=>{
	try {
		console.log("userExist: wallet path")
		...
		console.error(`Identity does not exist  : ${error}`);
		return error
	}
